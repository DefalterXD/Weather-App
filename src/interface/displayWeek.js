...
export const initDisplayWeekData = function initDisplayWeekDataOnThePage(
  daysOfTheWeek,
) {
  ...
  for (const day of daysOfTheWeek) {
    ...
    import(`../iconModules/${day.icon}.js`)
      .then((iconResult) => {
    ...
    weekTempMax.textContent = !tempMode.isCelsius
      ? `${day.tempmax} F°`
      : `${day.tempmax} C°`;
    ...
    weekTempMin.textContent = !tempMode.isCelsius
      ? `${day.tempmin} F°`
      : `${day.tempmin} C°`;
...
export const displayExistingWeekData =
  function displayExistingWeekDataOnThePage(daysOfTheWeek) {
    ...
    for (const maxTempCard of weekTempMax) {
      maxTempCard.textContent = !tempMode.isCelsius
        ? `${daysOfTheWeek[maxTempIdx].tempmax} F°`
        : `${daysOfTheWeek[maxTempIdx].tempmax} C°`;
      maxTempIdx++;
    }

    let weekImgIdx = 0;
    for (const weekImg of weekIconImgs) {
      import(`../iconModules/${daysOfTheWeek[weekImgIdx].icon}.js`)
        .then((iconResult) => {
    ...
    for (const minTempCard of weekTempMin) {
      minTempCard.textContent = !tempMode.isCelsius
        ? `${daysOfTheWeek[minTempIdx].tempmin} F°`
        : `${daysOfTheWeek[minTempIdx].tempmin} C°`;
      minTempIdx++;
    }
  };
